...
const areaConfig=(p,area)=>{
  if(!p) return {unit:'pcs',perBase:1};
  return area==='kasir'
...
function useStock(storeId){
 ...
 async function load(){
  ...
  const [{data:b,error:be},{data:p,error:pe}]=await Promise.all([
   ...
   supabase.from('products').select('id,code,name,category,unit,base_unit,kasir_unit,kasir_per_base,kitchen_unit,kitchen_per_base,min_stock,max_stock').eq('active',true).order('name')
  ]);
  if(be||pe)setError((be||pe).message);
  setRows(b||[]);setProducts(p||[]);setLoading(false)
 }
 ...
 const merged=useMemo(()=>products.map(p=>{
  ...
  return {
   ...p,
   gudang_qty:Number(b.gudang_qty||0),
   operasional_qty:Number(b.operasional_qty||0),
   kasir_qty:Number(b.kasir_qty||0),
   kitchen_qty:Number(b.kitchen_qty||0)
  }
 }),[products,rows]);
...
function Warehouse({storeId,role}){
 const {items,loading,error}=useStock(storeId);
 return <Page title="Stok Gudang" subtitle="Gudang + Operasional (Kasir & Kitchen digabung)">
  <div className="notice">Stok Operasional adalah satu saldo gabungan Kasir + Kitchen. Input sisa terbaru akan menggantikan saldo operasional sebelumnya.</div>
  <div className="table-wrap">{loading?<p>Memuat stok…</p>:error?<div className="error">{error}</div>:
   <table><thead><tr><th>Produk</th><th>Base</th><th>Stok Gudang</th><th>Stok Operasional</th><th>Total Stok</th><th>Status</th></tr></thead>
   <tbody>{items.map(x=>{const bu=baseUnit(x);const total=x.gudang_qty+x.operasional_qty;return <tr key={x.id}>
    <td><b>{x.name}</b><small>{x.code}</small></td><td>{bu}</td><td>{fmt(x.gudang_qty)} {bu}</td><td><b>{fmt(x.operasional_qty)} {bu}</b></td><td><b>{fmt(total)} {bu}</b></td><td><Status item={x}/></td>
   </tr>})}</tbody></table>}
  </div>{role&&<div className="notice">Role aktif: {role}</div>}
 </Page>
}
...
function Transfer({storeId}){
 ...
 const cfg=areaConfig(selected,operationalArea);
 const amount=Number(qty||0);
 const baseEquivalent=selected?areaToBase(amount,selected,operationalArea):0;
 const invalid=!!selected&&(amount<=0||baseEquivalent>Number(selected.gudang_qty||0)+1e-9);
 async function submit(e){
  e.preventDefault();setMsg('');
  if(!selected)return setMsg('Pilih produk terlebih dahulu.');
  if(amount<=0)return setMsg('Jumlah transfer harus lebih dari 0.');
  if(baseEquivalent>selected.gudang_qty+1e-9)return setMsg('Jumlah transfer melebihi stok gudang.');
  setSaving(true);
  const {error}=await supabase.rpc('record_stock_transfer_v2',{
   p_store_id:storeId,p_product_id:productId,p_area_qty:amount,p_note:note.trim()||null,p_operational_area:operationalArea
  });
  if(error)setMsg(error.message);
  else{setMsg('Transfer berhasil. Stok gudang dan operasional sudah diperbarui.');setQty('');setNote('');await reload()}
  setSaving(false)
 }
 return <Page title="Transfer Stok" subtitle="Gudang → Operasional">
  <div className="notice">Jumlah transfer mengikuti satuan area. Sistem otomatis mengonversinya ke satuan Base.</div>
  <form className="card form" onSubmit={submit}>
   <label>Tujuan Operasional<select value={operationalArea} onChange={e=>{setOperationalArea(e.target.value);setQty('')}} required><option value="kasir">Kasir</option><option value="kitchen">Kitchen</option></select></label>
   ...
   {selected&&<div className="stock-preview">
    <div><small>STOK GUDANG</small><b>{fmt(selected.gudang_qty)} {baseUnit(selected)}</b></div><div>→</div>
    <div><small>STOK {operationalArea==='kasir'?'KASIR':'KITCHEN'}</small><b>{fmt(operationalArea==='kasir'?selected.kasir_qty:selected.kitchen_qty)} {cfg.unit}</b><small>Konversi: 1 {baseUnit(selected)} = {fmt(cfg.perBase)} {cfg.unit}</small></div>
   </div>}
   <label>Jumlah Transfer ({cfg.unit})<input type="number" min="0.001" step="0.001" value={qty} onChange={e=>setQty(e.target.value)} required placeholder={`Contoh: ${operationalArea==='kasir'?'12':'5'}`}/></label>
   {selected&&<div className={invalid?'error':'hint'}>{invalid?'Jumlah melebihi stok gudang.':<>Setara <b>{fmt(baseEquivalent)} {baseUnit(selected)}</b> · Maksimal <b>{fmt(baseToArea(selected.gudang_qty,selected,operationalArea))} {cfg.unit}</b></>}</div>}
...
function Opname({storeId}){
 const {items,loading,reload}=useStock(storeId);
 const [productId,setProductId]=useState(''),[physical,setPhysical]=useState(''),[waste,setWaste]=useState('0'),[note,setNote]=useState(''),[saving,setSaving]=useState(false),[result,setResult]=useState(null),[msg,setMsg]=useState('');
 const selected=items.find(x=>x.id===productId);
 async function submit(e){
  e.preventDefault();setMsg('');setResult(null);
  if(!selected)return setMsg('Pilih produk terlebih dahulu.');
  const qty=Number(physical),w=Number(waste||0);
  if(!Number.isFinite(qty)||qty<0)return setMsg('Sisa stok operasional tidak boleh kurang dari 0.');
  if(!Number.isFinite(w)||w<0)return setMsg('Waste tidak boleh kurang dari 0.');
  setSaving(true);
  try{
   const {data,error}=await supabase.rpc('record_operational_stock_snapshot',{p_store_id:storeId,p_product_id:productId,p_physical_qty:qty,p_waste_qty:w,p_note:note.trim()||null});
   if(error)throw error;
   setResult(data);setPhysical('');setWaste('0');setNote('');await reload();setMsg('Sisa stok operasional berhasil diperbarui.');
  }catch(err){setMsg(err?.message||'Gagal menyimpan sisa stok.')}finally{setSaving(false)}
 }
 return <Page title="Input Sisa" subtitle="Ganti sisa stok operasional terbaru">
  <div className="notice">Masukkan <b>total sisa fisik Kasir + Kitchen</b>. Nilai ini langsung menjadi Stok Operasional terbaru dan menggantikan saldo operasional sebelumnya.</div>
  <form className="card form" onSubmit={submit}>
   <label>Produk<select value={productId} onChange={e=>{setProductId(e.target.value);setPhysical('');setMsg('');setResult(null)}} required><option value="">Pilih produk</option>{items.map(x=><option key={x.id} value={x.id}>{x.name} — {baseUnit(x)}</option>)}</select></label>
   {selected&&<div className="stock-preview"><div><small>STOK GUDANG</small><b>{fmt(selected.gudang_qty)} {baseUnit(selected)}</b></div><div>+</div><div><small>OPERASIONAL SAAT INI</small><b>{fmt(selected.operasional_qty)} {baseUnit(selected)}</b><small>Kasir + Kitchen</small></div></div>}
   <label>Sisa Operasional Terbaru ({selected?baseUnit(selected):'Base Unit'})<input type="number" min="0" step="0.001" value={physical} onChange={e=>setPhysical(e.target.value)} required placeholder="Contoh: 6"/></label>
   <div className="hint">Nilai sisa di atas adalah <b>gabungan Kasir + Kitchen</b>, bukan input terpisah per area.</div>
   <label>Waste (opsional, {selected?baseUnit(selected):'Base Unit'})<input type="number" min="0" step="0.001" value={waste} onChange={e=>setWaste(e.target.value)}/></label>
   <label>Catatan<input value={note} onChange={e=>setNote(e.target.value)} placeholder="Opsional"/></label>
   <button disabled={saving||loading}>{saving?'Menyimpan…':'✓ Simpan Sisa Stok'}</button>
   {msg&&<div className={msg.includes('berhasil')?'success':'error'}>{msg}</div>}
   {result&&<div className="result"><b>Stok operasional diperbarui</b><div>Sisa terbaru: {fmt(result.operational_after)} {selected?baseUnit(selected):''}</div><div>Stok Gudang: {fmt(result.gudang_qty)} {selected?baseUnit(selected):''}</div><div>Stok Akhir: {fmt(result.ending_qty)} {selected?baseUnit(selected):''}</div><div>Pemakaian: {fmt(result.usage_qty)} {selected?baseUnit(selected):''}</div><div>No. Transaksi: {result.transaction_no}</div></div>}
  </form>
 </Page>
}

function History({storeId}){const [rows,setRows]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState('');async function load(){if(!storeId)return;setLoading(true);const {data,error}=await supabase.from('stock_transactions').select('id,product_id,area,transaction_type,qty,note,created_at,transaction_no,operational_area,products(name,unit),profiles(full_name)').eq('store_id',storeId).order('created_at',{ascending:false}).limit(50);if(error)setError(error.message);setRows(data||[]);setLoading(false)}useEffect(()=>{load()},[storeId]);useEffect(()=>{if(!storeId)return;const channel=supabase.channel('rcm-history-'+storeId).on('postgres_changes',{event:'INSERT',schema:'public',table:'stock_transactions',filter:'store_id=eq.'+storeId},load).subscribe();return()=>supabase.removeChannel(channel)},[storeId]);return <Page title="Riwayat" subtitle="50 transaksi terbaru"><div className="history">{loading?<p>Memuat riwayat…</p>:error?<div className="error">{error}</div>:rows.length===0?<p>Belum ada transaksi.</p>:rows.map(r=><div className="history-row" key={r.id}><div><b>{r.products?.name||'Produk'}</b><small>{friendlyType(r.transaction_type)} · {r.area==='operasional'?'Operasional':'Gudang'}{r.transaction_no?' · '+r.transaction_no:''}</small></div><strong className={Number(r.qty)<0?'negative':'positive'}>{Number(r.qty)>0?'+':''}{fmt(r.qty)} {r.products?.unit||''}</strong><time>{new Date(r.created_at).toLocaleString('id-ID')}</time></div>)}</div></Page>}
function CrewStock({storeId}){
 const {items,loading,error}=useStock(storeId);
 const low=items.filter(x=>x.operasional_qty<=Number(x.min_stock||0));
 return <Page title="Stok" subtitle="Stok operasional gabungan Kasir + Kitchen">
  {error&&<div className="error">{error}</div>}
  ...
  <div className="crew-stock-list">{loading?<div className="empty">Memuat stok…</div>:items.length===0?<div className="empty">Belum ada produk aktif.</div>:items.map(x=>{const isLow=x.operasional_qty<=Number(x.min_stock||0);const bu=baseUnit(x);return <div className={'crew-stock-card '+(isLow?'is-low':'')} key={x.id}>
   <div className="crew-product"><b>{x.name}</b><small>{x.code} · {bu}</small></div>
   <div className="crew-stock-values"><div><span>Gudang</span><strong>{fmt(x.gudang_qty)} {bu}</strong></div><div><span>Operasional</span><strong>{fmt(x.operasional_qty)} {bu}</strong><small>Kasir + Kitchen</small></div><div><span>Total</span><strong>{fmt(x.gudang_qty+x.operasional_qty)} {bu}</strong></div></div>
   <span className={'badge '+(isLow?'danger':'ok')}>{isLow?'Menipis':'Normal'}</span>
  </div>})}</div>
 </Page>
}
function CrewOpname({storeId}){
 const {items,loading,reload}=useStock(storeId);
 const [productId,setProductId]=useState(''),[physical,setPhysical]=useState(''),[waste,setWaste]=useState('0'),[saving,setSaving]=useState(false),[msg,setMsg]=useState(''),[result,setResult]=useState(null);
 const selected=items.find(x=>x.id===productId);
 async function submit(e){
  e.preventDefault();setMsg('');setResult(null);
  if(!selected)return setMsg('Pilih produk terlebih dahulu.');
  ...
  if(!Number.isFinite(w)||w<0)return setMsg('Waste tidak boleh kurang dari 0.');
  setSaving(true);
  try{const {data,error}=await supabase.rpc('record_operational_stock_snapshot',{p_store_id:storeId,p_product_id:productId,p_physical_qty:qty,p_waste_qty:w,p_note:null});if(error)throw error;setResult(data);setMsg('Sisa stok berhasil diperbarui.');setPhysical('');setWaste('0');await reload()}catch(err){setMsg(err?.message||'Gagal menyimpan sisa stok.')}finally{setSaving(false)}
 }
 return <Page title="Input Sisa" subtitle="Ganti sisa operasional terbaru">
  <div className="crew-tip">💡 Hitung seluruh sisa <b>Kasir + Kitchen</b>, lalu masukkan sebagai satu angka. Sisa terbaru otomatis menggantikan saldo operasional sebelumnya.</div>
  <form className="card form crew-form" onSubmit={submit}>
   <label>1. Pilih Produk<select value={productId} onChange={e=>{setProductId(e.target.value);setPhysical('');setMsg('');setResult(null)}} required><option value="">Pilih produk</option>{items.map(x=><option key={x.id} value={x.id}>{x.name} — {baseUnit(x)}</option>)}</select></label>
   {selected&&<div className="crew-current"><span>Operasional saat ini · Kasir + Kitchen</span><b>{fmt(selected.operasional_qty)} {baseUnit(selected)}</b></div>}
   <label>2. Sisa Operasional Terbaru ({selected?baseUnit(selected):'Base Unit'})<input className="crew-big-input" type="number" min="0" step="0.001" value={physical} onChange={e=>setPhysical(e.target.value)} required placeholder="Contoh: 6"/></label>
   <div className="hint">Sisa terbaru ini menjadi <b>Stok Operasional</b> dan tidak dijumlahkan dengan sisa sebelumnya.</div>
   <details className="crew-optional"><summary>Tambah waste (opsional)</summary><label>Waste ({selected?baseUnit(selected):'Base Unit'})<input type="number" min="0" step="0.001" value={waste} onChange={e=>setWaste(e.target.value)}/></label></details>
   <button className="crew-save" disabled={saving||loading}>{saving?'Menyimpan…':'✓ Simpan Sisa Stok'}</button>{msg&&<div className={msg.includes('berhasil')?'success':'error'}>{msg}</div>}
   {result&&<div className="result"><b>Berhasil</b><div>Operasional: {fmt(result.operational_after)} {baseUnit(selected)}</div><div>Stok Akhir: {fmt(result.ending_qty)} {baseUnit(selected)}</div><div>Pemakaian: {fmt(result.usage_qty)} {baseUnit(selected)}</div><div>No. Transaksi: {result.transaction_no}</div></div>}
  </form>
 </Page>
}
...
function Products(){
 ...
 const blank={id:null,code:'',name:'',category:'',unit:'kg',base_unit:'kg',kasir_unit:'kg',kasir_per_base:1,kitchen_unit:'kg',kitchen_per_base:1,min_stock:0,max_stock:0,active:true};
 ...
 useEffect(()=>{load()},[]);
 function edit(r){setForm({...blank,...r,base_unit:r.base_unit||r.unit||'pcs',kasir_unit:r.kasir_unit||r.base_unit||r.unit||'pcs',kasir_per_base:Number(r.kasir_per_base||1),kitchen_unit:r.kitchen_unit||r.base_unit||r.unit||'pcs',kitchen_per_base:Number(r.kitchen_per_base||1)})}
 function reset(){setForm(blank)}
 function setField(k,v){setForm(f=>({...f,[k]:v}))}
 async function save(e){
  e.preventDefault();setSaving(true);setError('');
  const payload={
   p_product_id:form.id,p_code:form.code.trim(),p_name:form.name.trim(),p_category:form.category?.trim()||null,
   p_unit:form.base_unit||form.unit||'pcs',p_min_stock:Number(form.min_stock||0),p_max_stock:Number(form.max_stock||0),p_active:!!form.active,
   p_base_unit:form.base_unit||form.unit||'pcs',p_kasir_unit:form.kasir_unit||form.base_unit||'pcs',p_kasir_per_base:Number(form.kasir_per_base||0),
   p_kitchen_unit:form.kitchen_unit||form.base_unit||'pcs',p_kitchen_per_base:Number(form.kitchen_per_base||0)
  };
  let result;
  if(form.id) result=await supabase.rpc('update_product_master',payload);
  else result=await supabase.from('products').insert({code:payload.p_code,name:payload.p_name,category:payload.p_category,unit:payload.p_unit,base_unit:payload.p_base_unit,kasir_unit:payload.p_kasir_unit,kasir_per_base:payload.p_kasir_per_base,kitchen_unit:payload.p_kitchen_unit,kitchen_per_base:payload.p_kitchen_per_base,min_stock:payload.p_min_stock,max_stock:payload.p_max_stock,active:payload.p_active});
  const {error}=result;if(error)setError(error.message);else{reset();await load()}setSaving(false)
 }
 ...
 return <Page title="Master Produk" subtitle="Kelola produk dan konversi satuan Kasir / Kitchen">
  <div className="split">
   <form className="card form" onSubmit={save}>
    <h3>{form.id?'Edit Produk':'Tambah Produk'}</h3>
    <label>Kode Produk<input value={form.code} onChange={e=>setField('code',e.target.value)} required/></label>
    <label>Nama Produk<input value={form.name} onChange={e=>setField('name',e.target.value)} required/></label>
    <label>Kategori<input value={form.category||''} onChange={e=>setField('category',e.target.value)}/></label>
    <h4>Satuan Dasar (Base)</h4>
    <label>Base Unit<select value={form.base_unit} onChange={e=>{setField('base_unit',e.target.value);if(!form.id){setForm(f=>({...f,base_unit:e.target.value,unit:e.target.value,kasir_unit:e.target.value,kitchen_unit:e.target.value}))}}} required>{UNIT_OPTIONS.map(u=><option key={u} value={u}>{u}</option>)}</select></label>
    <div className="notice">Base adalah satuan standar untuk perhitungan stok gabungan. Contoh: 12 pcs Kasir = 1 kg Base.</div>
    <h4>Konversi Kasir</h4>
    <div className="two"><label>Satuan Kasir<select value={form.kasir_unit} onChange={e=>setField('kasir_unit',e.target.value)} required>{UNIT_OPTIONS.map(u=><option key={u} value={u}>{u}</option>)}</select></label><label>1 {form.base_unit} =<input type="number" min="0.000001" step="0.001" value={form.kasir_per_base} onChange={e=>setField('kasir_per_base',e.target.value)} required/></label></div>
    <h4>Konversi Kitchen</h4>
    <div className="two"><label>Satuan Kitchen<select value={form.kitchen_unit} onChange={e=>setField('kitchen_unit',e.target.value)} required>{UNIT_OPTIONS.map(u=><option key={u} value={u}>{u}</option>)}</select></label><label>1 {form.base_unit} =<input type="number" min="0.000001" step="0.001" value={form.kitchen_per_base} onChange={e=>setField('kitchen_per_base',e.target.value)} required/></label></div>
    <div className="two"><label>Min Stock<input type="number" min="0" step="0.001" value={form.min_stock} onChange={e=>setField('min_stock',e.target.value)}/></label><label>Max Stock<input type="number" min="0" step="0.001" value={form.max_stock} onChange={e=>setField('max_stock',e.target.value)}/></label></div>
    <label className="check"><input type="checkbox" checked={!!form.active} onChange={e=>setField('active',e.target.checked)}/> Produk aktif</label>
    <div className="actions"><button disabled={saving}>{saving?'Menyimpan…':form.id?'Simpan Perubahan':'Tambah Produk'}</button>{form.id&&<button type="button" className="secondary" onClick={reset}>Batal</button>}</div>
    {error&&<div className="error">{error}</div>}
   </form>
   <div className="table-wrap">{loading?<p>Memuat produk…</p>:rows.length===0?<p>Belum ada produk. Tambahkan produk pertama.</p>:
    <table><thead><tr><th>Produk</th><th>Base</th><th>Kasir</th><th>Kitchen</th><th>Min/Max</th><th>Status</th><th></th></tr></thead>
    <tbody>{rows.map(r=>{
     const bu=baseUnit(r),kc=areaConfig(r,'kasir'),ki=areaConfig(r,'kitchen');
     return <tr key={r.id}><td><b>{r.name}</b><small>{r.code}</small></td><td>{bu}</td><td>1 {bu} = {fmt(kc.perBase)} {kc.unit}</td><td>1 {bu} = {fmt(ki.perBase)} {ki.unit}</td><td>{fmt(r.min_stock)} / {fmt(r.max_stock)}</td><td>{r.active?<span className="badge ok">Aktif</span>:<span className="badge danger">Nonaktif</span>}</td><td><button className="smallbtn" onClick={()=>edit(r)}>Edit</button><button className="smallbtn" onClick={()=>toggle(r)}>{r.active?'Nonaktifkan':'Aktifkan'}</button></td></tr>
    })}</tbody></table>}</div>
  </div>
 </Page>
}
