...
function Transfer({storeId}){
 ...
 return <Page title="Transfer Stok" subtitle="Gudang → Operasional"><div className="notice">Pilih tujuan Kasir atau Kitchen. Saldo operasional di aplikasi tetap dihitung sebagai satu total gabungan Kasir + Kitchen.</div><form className="card form" onSubmit={submit}>
  ...
  {selected&&<div className="stock-preview"><div><small>STOK GUDANG</small><b>{fmt(selected.gudang_qty)} {baseUnit(selected)}</b></div><div>→</div><div><small>TOTAL OPERASIONAL</small><b>{fmt(selected.operasional_qty)} {baseUnit(selected)}</b><small>Tujuan: {operationalArea==='kasir'?'Kasir':'Kitchen'}</small></div></div>}
  ...
  {selected&&<div className={invalid?'error':'hint'}>{invalid?'Jumlah tidak boleh melebihi stok gudang.':<>Maksimal transfer: <b>{fmt(selected.gudang_qty)} {baseUnit(selected)}</b></>}</div>}
...
function History({storeId}){const [rows,setRows]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState('');async function load(){if(!storeId)return;setLoading(true);const {data,error}=await supabase.from('stock_transactions').select('id,product_id,area,transaction_type,qty,note,created_at,created_by,products(name,unit)').eq('store_id',storeId).order('created_at',{ascending:false}).limit(50);if(error)setError(error.message);setRows(data||[]);setLoading(false)}useEffect(()=>{load()},[storeId]);useEffect(()=>{if(!storeId)return;const channel=supabase.channel('rcm-history-'+storeId).on('postgres_changes',{event:'INSERT',schema:'public',table:'stock_transactions',filter:'store_id=eq.'+storeId},load).subscribe();return()=>supabase.removeChannel(channel)},[storeId]);return <Page title="Riwayat" subtitle="50 transaksi terbaru"><div className="history">{loading?<p>Memuat riwayat…</p>:error?<div className="error">{error}</div>:rows.length===0?<p>Belum ada transaksi.</p>:rows.map(r=><div className="history-row" key={r.id}><div><b>{r.products?.name||'Produk'}</b><small>{r.transaction_type} · {r.area}</small></div><strong className={Number(r.qty)<0?'negative':'positive'}>{Number(r.qty)>0?'+':''}{fmt(r.qty)} {r.products?.unit||''}</strong><time>{new Date(r.created_at).toLocaleString('id-ID')}</time></div>)}</div></Page>}
function CrewStock({storeId}){
 const {items,loading,error}=useStock(storeId);
 const low=items.filter(x=>x.ending_qty<=Number(x.min_stock||0));
 return <Page title="Stok" subtitle="Stok awal, pemakaian, dan stok akhir">
  {error&&<div className="error">{error}</div>}
...
function Products(){
 ...
 const blank={id:null,code:'',name:'',category:'',unit:'kg',base_unit:'kg',kasir_unit:'pcs',kasir_per_base:12,kitchen_unit:'kg',kitchen_per_base:1,min_stock:0,max_stock:0,active:true};
 const [form,setForm]=useState(blank);
 async function load(){setLoading(true);setError('');try{const {data,error}=await supabase.from('products').select('*').order('name');if(error)throw error;setRows(data||[])}catch(err){setError(err?.message||'Gagal memuat master produk.')}finally{setLoading(false)}}
 useEffect(()=>{load()},[]);
 function setField(k,v){setForm(prev=>({...prev,[k]:v}))}
 function edit(r){setForm({...blank,...r,base_unit:r.base_unit||r.unit||'kg',kasir_unit:r.kasir_unit||r.unit||'pcs',kasir_per_base:Number(r.kasir_per_base||1),kitchen_unit:r.kitchen_unit||r.unit||'kg',kitchen_per_base:Number(r.kitchen_per_base||1)})}
 function reset(){setForm(blank)}
 async function save(e){
  e.preventDefault();setSaving(true);setError('');
  try{
   const min=Number(form.min_stock||0),max=Number(form.max_stock||0),kp=Number(form.kasir_per_base||0),kp2=Number(form.kitchen_per_base||0);
   if(!form.code.trim()||!form.name.trim())throw new Error('Kode dan nama produk wajib diisi.');
   if(min<0||max<0)throw new Error('Min/Max Stock tidak boleh kurang dari 0.');
   if(max>0&&max<min)throw new Error('Max Stock tidak boleh lebih kecil dari Min Stock.');
   if(kp<=0||kp2<=0)throw new Error('Konversi Kasir dan Kitchen harus lebih dari 0.');
   const payload={p_product_id:form.id,p_code:form.code.trim(),p_name:form.name.trim(),p_category:form.category?.trim()||null,p_unit:form.base_unit||form.unit||'kg',p_min_stock:min,p_max_stock:max,p_active:!!form.active,p_base_unit:form.base_unit||form.unit||'kg',p_kasir_unit:form.kasir_unit||form.base_unit||form.unit||'kg',p_kasir_per_base:kp,p_kitchen_unit:form.kitchen_unit||form.base_unit||form.unit||'kg',p_kitchen_per_base:kp2};
   let result;
   if(form.id){
    result=await supabase.rpc('update_product_master',{p_product_id:form.id,p_code:payload.p_code,p_name:payload.p_name,p_category:payload.p_category,p_unit:payload.p_unit,p_min_stock:min,p_max_stock:max,p_active:payload.p_active});
    if(result.error)throw result.error;
    const {error:convError}=await supabase.from('products').update({base_unit:payload.p_base_unit,kasir_unit:payload.p_kasir_unit,kasir_per_base:payload.p_kasir_per_base,kitchen_unit:payload.p_kitchen_unit,kitchen_per_base:payload.p_kitchen_per_base}).eq('id',form.id);
    if(convError)throw convError;
   }else{
    const insertPayload={code:payload.p_code,name:payload.p_name,category:payload.p_category,unit:payload.p_unit,base_unit:payload.p_base_unit,kasir_unit:payload.p_kasir_unit,kasir_per_base:payload.p_kasir_per_base,kitchen_unit:payload.p_kitchen_unit,kitchen_per_base:payload.p_kitchen_per_base,min_stock:min,max_stock:max,active:payload.p_active};
    result=await supabase.from('products').insert(insertPayload);
    if(result.error)throw result.error;
   }
   reset();await load();
  }catch(err){setError(err?.message||'Gagal menyimpan produk.')}finally{setSaving(false)}
 }
 ...
 return <Page title="Master Produk" subtitle="Kelola produk, satuan, konversi dan batas stok">
  <div className="split">
   <form className="card form" onSubmit={save}>
    ...
    <label>Satuan Base<select value={form.base_unit||form.unit} onChange={e=>{setField('base_unit',e.target.value);setField('unit',e.target.value)}} required>{UNIT_OPTIONS.map(u=><option key={u} value={u}>{u}</option>)}</select></label>
    ...
    <div className="actions"><button disabled={saving}>{saving?'Menyimpan…':form.id?'Simpan Perubahan':'Tambah Produk'}</button>{form.id&&<button type="button" className="secondary" onClick={reset} disabled={saving}>Batal</button>}</div>
    {error&&<div className="error">{error}</div>}
   </form>
   <div className="table-wrap">{loading?<p>Memuat produk…</p>:rows.length===0?<p>Belum ada produk. Tambahkan produk pertama.</p>:<table><thead><tr><th>Produk</th><th>Kategori</th><th>Base</th><th>Kasir</th><th>Kitchen</th><th>Min/Max</th><th>Status</th><th></th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.name}</b><small>{r.code}</small></td><td>{r.category||'-'}</td><td>{r.base_unit||r.unit}</td><td>1 {(r.base_unit||r.unit)} = {fmt(r.kasir_per_base||1)} {r.kasir_unit||r.unit}</td><td>1 {(r.base_unit||r.unit)} = {fmt(r.kitchen_per_base||1)} {r.kitchen_unit||r.unit}</td><td>{fmt(r.min_stock)} / {fmt(r.max_stock)}</td><td>{r.active?<span className="badge ok">Aktif</span>:<span className="badge danger">Nonaktif</span>}</td><td><button className="smallbtn" onClick={()=>edit(r)} disabled={saving}>Edit</button><button className="smallbtn" onClick={()=>toggle(r)} disabled={saving}>{r.active?'Nonaktifkan':'Aktifkan'}</button></td></tr>)}</tbody></table>}</div>
  </div>
 </Page>
}
