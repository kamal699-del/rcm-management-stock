...
function Transfer({storeId}={}){
 ...
 const unit=selected?baseUnit(selected):'pcs';
 const available=Number(selected?.gudang_qty||0);
 const invalid=!!selected&&(!Number.isFinite(amount)||amount<=0||amount>available+1e-9);
 async function submit(e){
  e.preventDefault(); setMsg('');
  if(!storeId)return setMsg('Store belum terdeteksi. Silakan keluar lalu masuk kembali.');
  if(!selected)return setMsg('Pilih produk terlebih dahulu.');
  if(!Number.isFinite(amount)||amount<=0)return setMsg('Jumlah transfer harus lebih dari 0.');
  if(amount>available+1e-9)return setMsg('Jumlah transfer melebihi stok gudang.');
  setSaving(true);
  try{
   const payload={p_store_id:storeId,p_product_id:productId,p_qty:amount,p_note:note.trim()||null,p_operational_area:operationalArea};
   let result=await supabase.rpc('record_stock_transfer',{...payload});
   if(result.error){
    const fallback=await supabase.rpc('record_stock_transfer_v2',{p_store_id:storeId,p_product_id:productId,p_area_qty:amount,p_note:note.trim()||null,p_operational_area:operationalArea});
    if(fallback.error)throw fallback.error;
   }
   setMsg('Transfer berhasil. Stok gudang dan operasional sudah diperbarui.');
   setQty('');setNote('');
   if(reload)await reload();
  }catch(err){setMsg(err?.message||'Terjadi kesalahan saat transfer.');}
  finally{setSaving(false);}
 }
 return <Page title="Transfer Stok" subtitle="Gudang → Operasional">
  <div className="notice">Jumlah transfer menggunakan <b>satuan Base</b>. Kasir/Kitchen hanya menentukan tujuan. Sistem menangani konversi satuan area sesuai Master Produk.</div>
  <form className="card form" onSubmit={submit}>
   ...
   {selected&&<div className="stock-preview"><div><small>STOK GUDANG</small><b>{fmt(available)} {unit}</b></div><div>→</div><div><small>TOTAL OPERASIONAL</small><b>{fmt(selected.operasional_qty)} {unit}</b><small>Tujuan: {operationalArea==='kasir'?'Kasir':'Kitchen'}</small></div></div>}
   <label>Jumlah Transfer ({unit})<input type="number" min="0.001" max={selected?available:undefined} step="0.001" value={qty} onChange={e=>setQty(e.target.value)} required placeholder={'Contoh: 1 '+unit}/></label>
   {selected&&<div className={invalid?'error':'hint'}>{invalid?'Jumlah tidak boleh melebihi stok gudang.':<>Maksimal transfer: <b>{fmt(available)} {unit}</b></>}</div>}
...
function History({storeId}){const [rows,setRows]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState('');async function load(){if(!storeId)return;setLoading(true);setError('');try{const {data,error}=await supabase.from('stock_transactions').select('id,transaction_no,product_id,area,transaction_type,qty,note,created_at,products(name,unit)').eq('store_id',storeId).order('created_at',{ascending:false}).limit(50);if(error)throw error;setRows(data||[])}catch(err){setError(err?.message||'Gagal memuat riwayat transaksi.')}finally{setLoading(false)}}useEffect(()=>{load()},[storeId]);useEffect(()=>{if(!storeId)return;const channel=supabase.channel('rcm-history-'+storeId).on('postgres_changes',{event:'INSERT',schema:'public',table:'stock_transactions',filter:'store_id=eq.'+storeId},load).subscribe();return()=>supabase.removeChannel(channel)},[storeId]);return <Page title="Riwayat" subtitle="50 transaksi terbaru"><div className="history">{loading?<p>Memuat riwayat…</p>:error?<div className="error">{error}</div>:rows.length===0?<p>Belum ada transaksi.</p>:rows.map(r=><div className="history-row" key={r.id}><div><b>{r.products?.name||'Produk'}</b><small>{r.transaction_no||'Tanpa nomor'} · {r.transaction_type} · {r.area||'-'}{r.note?' · '+r.note:''}</small></div><strong className={Number(r.qty)<0?'negative':'positive'}>{Number(r.qty)>0?'+':''}{fmt(r.qty)} {r.products?.unit||''}</strong><time>{new Date(r.created_at).toLocaleString('id-ID')}</time></div>)}</div></Page>}
