...
function useStock(storeId){
 ...
 async function load(){
  ...
  try{
   const {data,error}=await supabase.from('stock_status_summary').select('*').eq('store_id',storeId).order('name');
   if(error)throw error;
   setRows(data||[]);
  }catch(e){setError(e?.message||'Gagal memuat stok.');setRows([])}
 ...
 const merged=useMemo(()=>rows.map(x=>({...x,gudang_qty:Number(x.gudang_qty||0),operasional_qty:Number(x.operasional_qty||0),stock_awal:Number(x.stock_awal||0),pemakaian:Number(x.pemakaian||0),stock_akhir:Number(x.stock_akhir||0)})),[rows]);
...
function Dashboard({storeId,setTab}){
 ...
 const low=items.filter(x=>x.stock_status==='Menipis');
 const totalG=items.reduce((s,x)=>s+x.gudang_qty,0); const totalO=items.reduce((s,x)=>s+x.operasional_qty,0);
 const totalEnd=items.reduce((s,x)=>s+x.stock_akhir,0); const totalUse=items.reduce((s,x)=>s+x.pemakaian,0);
 ...
 return <div className="leader-dashboard">
  ...
  <div className="kpi-grid">
   ...
   <Kpi icon="▤" label="Stok Akhir" value={loading?'…':fmt(totalEnd)} meta="Gudang + Operasional"/>
  ...
  <div className="dash-grid">
   <div className="panel critical"><div className="panel-head"><div><h3>⚠ Produk perlu perhatian</h3><span>{low.length} produk di bawah / sama dengan minimum</span></div><button className="linkbtn" onClick={()=>setTab('warehouse')}>Lihat semua</button></div>
    {low.length===0?<div className="empty">Tidak ada produk kritis. 👍</div>:<div className="critical-list">{low.slice(0,6).map(x=><div className="critical-row" key={x.product_id}><div><b>{x.name}</b><small>{x.code} · Min {fmt(x.min_stock)} {x.base_unit||x.unit}</small></div><strong>{fmt(x.stock_akhir)} {x.base_unit||x.unit}</strong></div>)}</div>}
   </div>
...
function StockIn({storeId}){
 const {items,loading,reload}=useStock(storeId);const [productId,setProductId]=useState(''),[qty,setQty]=useState(''),[note,setNote]=useState(''),[saving,setSaving]=useState(false),[msg,setMsg]=useState('');const selected=items.find(x=>x.product_id===productId);
 async function submit(e){e.preventDefault();setSaving(true);setMsg('');try{const amount=Number(qty);if(!productId)throw new Error('Pilih produk terlebih dahulu.');if(!Number.isFinite(amount)||amount<=0)throw new Error('Jumlah stok harus lebih dari 0.');const {error}=await supabase.rpc('record_stock_in',{p_store_id:storeId,p_product_id:productId,p_qty:amount,p_note:note||null});if(error)throw error;setMsg('Stok gudang berhasil ditambahkan.');setQty('');setNote('');await reload()}catch(e){setMsg(e?.message||'Gagal menambah stok gudang.')}finally{setSaving(false)}}
 return <Page title="Input Stok Gudang" subtitle="Tambah stok masuk ke gudang"><form className="card form" onSubmit={submit}><label>Produk<select value={productId} onChange={e=>setProductId(e.target.value)} required><option value="">Pilih produk</option>{items.map(x=><option key={x.product_id} value={x.product_id}>{x.name} — stok akhir {fmt(x.stock_akhir)} {x.base_unit||x.unit}</option>)}</select></label>{selected&&<div className="hint">Stok akhir saat ini: <b>{fmt(selected.stock_akhir)} {selected.base_unit||selected.unit}</b></div>}<label>Jumlah stok masuk<input type="number" min="0.001" step="0.001" value={qty} onChange={e=>setQty(e.target.value)} required placeholder="Contoh: 10"/></label><label>Catatan<input value={note} onChange={e=>setNote(e.target.value)} placeholder="Contoh: Penerimaan supplier"/></label><button disabled={saving||loading}>{saving?'Menyimpan…':'＋ Tambah Stok Gudang'}</button>{msg&&<div className={msg.includes('berhasil')?'success':'error'}>{msg}</div>}</form></Page>
}
function StockColumns({item}){
 return <>
  <td>{fmt(item.stock_awal)} {item.base_unit||item.unit}</td>
  <td className={Number(item.pemakaian)>0?'negative':''}>{fmt(item.pemakaian)} {item.base_unit||item.unit}</td>
  <td><b>{fmt(item.stock_akhir)} {item.base_unit||item.unit}</b></td>
 </>;
}
function Warehouse({storeId,role}){
 const {items,loading,error}=useStock(storeId);
 return <Page title="Stok Gudang" subtitle="Stok gabungan dengan ringkasan awal, pemakaian dan akhir">
  <div className="notice">Stok Akhir = Gudang + Stok Operasional. Stok Awal otomatis mengambil Stok Akhir sebelumnya, sedangkan Pemakaian = Stok Awal − Stok Akhir.</div>
  <div className="table-wrap">{loading?<p>Memuat stok…</p>:error?<div className="error">{error}</div>:<table><thead><tr><th>Produk</th><th>Satuan</th><th>Stok Awal</th><th>Pemakaian</th><th>Stok Akhir</th><th>Status</th></tr></thead><tbody>{items.map(x=><tr key={x.product_id||x.id}><td><b>{x.name}</b><small>{x.code}</small></td><td>{x.base_unit||x.unit}</td><StockColumns item={x}/><td><span className={'badge '+(x.stock_status==='Menipis'?'danger':x.stock_status==='Penuh'?'warn':'ok')}>{x.stock_status||'Normal'}</span></td></tr>)}</tbody></table>}</div>{role&&<div className="notice">Role aktif: {role}</div>}
 </Page>;
}
...
function Transfer({storeId}){
 ...
 const selected=items.find(x=>x.product_id===productId); const amount=Number(qty||0); const invalid=!!selected&&(amount<=0||amount>selected.gudang_qty);
 ...
 return <Page title="Transfer Stok" subtitle="Gudang → Operasional"><div className="notice">Pilih tujuan Kasir atau Kitchen. Saldo operasional di aplikasi tetap dihitung sebagai satu total gabungan Kasir + Kitchen.</div><form className="card form" onSubmit={submit}>
  ...
  <label>Produk<select value={productId} onChange={e=>{setProductId(e.target.value);setQty('');setMsg('')}} required><option value="">Pilih produk</option>{items.map(x=><option key={x.product_id} value={x.product_id}>{x.name} — gudang {fmt(x.gudang_qty)} {x.unit}</option>)}</select></label>
...
function Opname({storeId}){
 ...
 const selected=items.find(x=>x.product_id===productId);
...
function History({storeId}){
 ...
 return <Page title="Riwayat" subtitle="100 transaksi terbaru"><div className="history">{loading?<div className="empty">Memuat riwayat…</div>:error?<div className="error">{error}</div>:rows.length===0?<div className="empty">Belum ada transaksi.</div>:rows.map(r=><div className="history-row" key={r.id}><div><b>{r.products?.name||'Produk'}</b><small>{r.transaction_no||'-'} · {friendlyType(r.transaction_type)} · {r.area}{r.operational_area?' · '+r.operational_area:''}</small><small>{r.creator_name}{r.note?' · '+r.note:''}</small></div><strong className={Number(r.qty)<0?'negative':'positive'}>{Number(r.qty)>0?'+':''}{fmt(r.qty)} {r.products?.unit||''}</strong><time>{new Date(r.created_at).toLocaleString('id-ID',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'})}</time></div>)}</div></Page>
}
function CrewStock({storeId}){
 const {items,loading,error}=useStock(storeId);
 const low=items.filter(x=>x.stock_status==='Menipis');
 return <Page title="Stok" subtitle="Stok dengan ringkasan awal, pemakaian dan akhir">
  {error&&<div className="error">{error}</div>}
  ...
  <div className="table-wrap">{loading?<div className="empty">Memuat stok…</div>:items.length===0?<div className="empty">Belum ada produk aktif.</div>:<table><thead><tr><th>Produk</th><th>Satuan</th><th>Stok Awal</th><th>Pemakaian</th><th>Stok Akhir</th><th>Status</th></tr></thead><tbody>{items.map(x=><tr key={x.product_id}><td><b>{x.name}</b><small>{x.code}</small></td><td>{x.base_unit||x.unit}</td><StockColumns item={x}/><td><span className={'badge '+(x.stock_status==='Menipis'?'danger':x.stock_status==='Penuh'?'warn':'ok')}>{x.stock_status||'Normal'}</span></td></tr>)}</tbody></table>}</div>
 </Page>;
}
function CrewOpname({storeId}){
 ...
 return <Page title="Input Sisa" subtitle="Masukkan sisa stok yang kamu hitung"><div className="crew-tip">💡 Pilih area yang dihitung. Total stok operasional di aplikasi tetap merupakan gabungan Kasir + Kitchen.</div><form className="card form crew-form" onSubmit={submit}><label>1. Pilih Area<select value={operationalArea} onChange={e=>setOperationalArea(e.target.value)} required><option value="kasir">Kasir</option><option value="kitchen">Kitchen</option></select></label><label>2. Pilih Produk<select value={productId} onChange={e=>setProductId(e.target.value)} required><option value="">Pilih produk</option>{items.map(x=><option key={x.product_id} value={x.product_id}>{x.name}</option>)}</select></label>{selected&&<div className="crew-current"><span>Sistem {operationalArea==='kasir'?'Kasir':'Kitchen'} · Total operasional</span><b>{fmt(areaSystem)} · {fmt(selected.operasional_qty)} {selected.unit}</b></div>}<label>3. Sisa Fisik<input className="crew-big-input" type="number" min="0" step="0.001" value={physical} onChange={e=>setPhysical(e.target.value)} required placeholder="Masukkan jumlah sisa"/></label><details className="crew-optional"><summary>Tambah waste (opsional)</summary><label>Waste<input type="number" min="0" step="0.001" value={waste} onChange={e=>setWaste(e.target.value)}/></label></details><button className="crew-save" disabled={saving||loading}>{saving?'Menyimpan…':'✓ Simpan Sisa Stok'}</button>{msg&&<div className={msg.includes('berhasil')?'success':'error'}>{msg}</div>}</form></Page>
}

function MyHistory({storeId}){
 const [rows,setRows]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState('');
 async function load(){
  if(!storeId)return;setLoading(true);setError('');
  try{const {data:user}=await supabase.auth.getUser();if(!user?.user){setRows([]);return}const {data,error}=await supabase.from('stock_transactions').select('id,area,transaction_type,qty,note,created_at,transaction_no,operational_area,products(name,unit)').eq('store_id',storeId).eq('created_by',user.user.id).order('created_at',{ascending:false}).limit(50);if(error)throw error;setRows(data||[])}catch(e){setError(e?.message||'Gagal memuat riwayat.');setRows([])}finally{setLoading(false)}}
 useEffect(()=>{load()},[storeId]);
 useEffect(()=>{if(!storeId)return;const channel=supabase.channel('rcm-my-history-'+storeId).on('postgres_changes',{event:'INSERT',schema:'public',table:'stock_transactions',filter:'store_id=eq.'+storeId},load).subscribe();return()=>supabase.removeChannel(channel)},[storeId]);
 return <Page title="Riwayat Saya" subtitle="Aktivitas stok yang kamu input"><div className="history">{loading?<p className="empty">Memuat riwayat…</p>:error?<div className="error">{error}</div>:rows.length===0?<p className="empty">Belum ada aktivitas stok dari akun ini.</p>:rows.map(r=><div className="history-row" key={r.id}><div><b>{r.products?.name||'Produk'}</b><small>{r.transaction_no||'-'} · {friendlyType(r.transaction_type)} · {r.area==='operasional'?'Operasional':'Gudang'}{r.operational_area?' · '+r.operational_area:''}</small><small>{r.note||'Tanpa catatan'}</small></div><strong className={Number(r.qty)<0?'negative':'positive'}>{Number(r.qty)>0?'+':''}{fmt(r.qty)} {r.products?.unit||''}</strong><time>{new Date(r.created_at).toLocaleString('id-ID',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</time></div>)}</div></Page>
}
...
function Products(){
 ...
 async function load(){setLoading(true);setError('');try{const {data,error}=await supabase.from('products').select('*').order('name');if(error)throw error;setRows(data||[])}catch(e){setError(e?.message||'Gagal memuat master produk.');setRows([])}finally{setLoading(false)}}
 useEffect(()=>{load()},[]);
 function edit(r){setForm({...blank,...r,kasir_per_base:r.kasir_per_base||1,kitchen_per_base:r.kitchen_per_base||1})}
 function reset(){setForm({...blank})}
 function valid(){const min=Number(form.min_stock),max=Number(form.max_stock),kp=Number(form.kasir_per_base),hp=Number(form.kitchen_per_base);if(!form.code.trim()||!form.name.trim())return 'Kode dan nama produk wajib diisi.';if(!Number.isFinite(min)||min<0)return 'Min Stock harus 0 atau lebih.';if(!Number.isFinite(max)||max<0)return 'Max Stock harus 0 atau lebih.';if(max>0&&max<min)return 'Max Stock tidak boleh lebih kecil dari Min Stock.';if(!Number.isFinite(kp)||kp<=0)return 'Konversi Kasir harus lebih dari 0.';if(!Number.isFinite(hp)||hp<=0)return 'Konversi Kitchen harus lebih dari 0.';return ''}
 async function save(e){e.preventDefault();setSaving(true);setError('');try{const validation=valid();if(validation)throw new Error(validation);const payload={code:form.code.trim(),name:form.name.trim(),category:form.category?.trim()||null,unit:form.base_unit||form.unit||'pcs',base_unit:form.base_unit||form.unit||'pcs',kasir_unit:form.kasir_unit||form.base_unit||form.unit||'pcs',kasir_per_base:Number(form.kasir_per_base||1),kitchen_unit:form.kitchen_unit||form.base_unit||form.unit||'pcs',kitchen_per_base:Number(form.kitchen_per_base||1),min_stock:Number(form.min_stock||0),max_stock:Number(form.max_stock||0),active:!!form.active};let result;if(form.id){result=await supabase.from('products').update(payload).eq('id',form.id)}else{result=await supabase.from('products').insert(payload)}if(result.error)throw result.error;reset();await load()}catch(e){console.error(e);setError(e?.message||'Gagal menyimpan produk.')}finally{setSaving(false)}}
 async function toggle(r){setSaving(true);setError('');try{const {error}=await supabase.rpc('set_product_active',{p_product_id:r.id,p_active:!r.active});if(error)throw error;await load()}catch(e){setError(e?.message||'Gagal mengubah status produk.')}finally{setSaving(false)}}
 const set=(k,v)=>setForm(x=>({...x,[k]:v}));
 return <Page title="Master Produk" subtitle="Kelola produk, satuan dan konversi Kasir/Kitchen"><div className="split"><form className="card form" onSubmit={save}><h3>{form.id?'Edit Produk':'Tambah Produk'}</h3><label>Kode Produk<input value={form.code} onChange={e=>set('code',e.target.value)} required/></label><label>Nama Produk<input value={form.name} onChange={e=>set('name',e.target.value)} required/></label><label>Kategori<input value={form.category||''} onChange={e=>set('category',e.target.value)}/></label><label>Satuan Dasar (Base Unit)<select value={form.base_unit||form.unit} onChange={e=>{set('base_unit',e.target.value);if(!form.id){set('kasir_unit',e.target.value);set('kitchen_unit',e.target.value)}}} required><option value="gr">gr</option><option value="kg">kg</option><option value="pcs">pcs</option></select></label><div className="notice"><b>Konversi Area</b><br/>Angka “per Base” berarti berapa unit area = 1 Base Unit.</div><div className="two"><label>Kasir Unit<select value={form.kasir_unit||form.base_unit||'pcs'} onChange={e=>set('kasir_unit',e.target.value)}><option value="gr">gr</option><option value="kg">kg</option><option value="pcs">pcs</option></select></label><label>Kasir per Base<input type="number" min="0.000001" step="0.001" value={form.kasir_per_base} onChange={e=>set('kasir_per_base',e.target.value)}/></label></div><div className="two"><label>Kitchen Unit<select value={form.kitchen_unit||form.base_unit||'pcs'} onChange={e=>set('kitchen_unit',e.target.value)}><option value="gr">gr</option><option value="kg">kg</option><option value="pcs">pcs</option></select></label><label>Kitchen per Base<input type="number" min="0.000001" step="0.001" value={form.kitchen_per_base} onChange={e=>set('kitchen_per_base',e.target.value)}/></label></div><div className="hint">Contoh Beras: Base = kg, Kasir = pcs, Kasir per Base = 12 → 12 pcs = 1 kg.</div><div className="two"><label>Min Stock<input type="number" min="0" step="0.001" value={form.min_stock} onChange={e=>set('min_stock',e.target.value)}/></label><label>Max Stock<input type="number" min="0" step="0.001" value={form.max_stock} onChange={e=>set('max_stock',e.target.value)}/></label></div><label className="check"><input type="checkbox" checked={!!form.active} onChange={e=>set('active',e.target.checked)}/> Produk aktif</label><div className="actions"><button disabled={saving}>{saving?'Menyimpan…':form.id?'Simpan Perubahan':'Tambah Produk'}</button>{form.id&&<button type="button" className="secondary" onClick={reset} disabled={saving}>Batal</button>}</div>{error&&<div className="error">{error}</div>}</form><div className="table-wrap">{loading?<p>Memuat produk…</p>:rows.length===0?<p>Belum ada produk. Tambahkan produk pertama.</p>:<table><thead><tr><th>Produk</th><th>Base</th><th>Kasir</th><th>Kitchen</th><th>Min/Max</th><th>Status</th><th></th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.name}</b><small>{r.code}</small></td><td>{r.base_unit||r.unit}</td><td>{r.kasir_unit||r.unit} = {fmt(r.kasir_per_base||1)} / Base</td><td>{r.kitchen_unit||r.unit} = {fmt(r.kitchen_per_base||1)} / Base</td><td>{fmt(r.min_stock)} / {fmt(r.max_stock)}</td><td>{r.active?<span className="badge ok">Aktif</span>:<span className="badge danger">Nonaktif</span>}</td><td><button className="smallbtn" onClick={()=>edit(r)} disabled={saving}>Edit</button><button className="smallbtn" onClick={()=>toggle(r)} disabled={saving}>{r.active?'Nonaktifkan':'Aktifkan'}</button></td></tr>)}</tbody></table>}</div></div></Page>
}
function Users(){
