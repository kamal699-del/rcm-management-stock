...
function ReviseWarehouseStock({storeId}){const {items,loading,reload}=useStock(storeId);const [productId,setProductId]=useState(''),[qty,setQty]=useState(''),[note,setNote]=useState(''),[saving,setSaving]=useState(false),[msg,setMsg]=useState('');const selected=items.find(x=>x.id===productId);useEffect(()=>{if(selected)setQty(String(selected.gudang_qty||0));},[productId]);async function submit(e){e.preventDefault();setSaving(true);setMsg('');const amount=Number(qty);if(!Number.isFinite(amount)||amount<0){setMsg('Stok baru harus 0 atau lebih.');setSaving(false);return}if(!selected){setMsg('Pilih produk terlebih dahulu.');setSaving(false);return}if(Math.abs(amount-Number(selected.gudang_qty||0))<0.0000001){setMsg('Tidak ada perubahan stok.');setSaving(false);return}if(!note.trim()){setMsg('Catatan revisi wajib diisi agar perubahan dapat ditelusuri.');setSaving(false);return}const {error}=await supabase.rpc('record_warehouse_stock_revision',{p_store_id:storeId,p_product_id:productId,p_new_qty:amount,p_note:note.trim()});if(error)setMsg(error.message);else{setMsg('Revisi stok gudang berhasil disimpan.');setNote('');await reload()}setSaving(false)}return <Page title="Revisi Stok Gudang" subtitle="Rubah stok gudang dengan catatan audit"><div className="notice">Menu ini hanya dapat diakses Admin, Store Leader, dan Team Leader. Setiap perubahan otomatis tercatat sebagai adjustment di Riwayat.</div><form className="card form" onSubmit={submit}><label>Produk<select value={productId} onChange={e=>setProductId(e.target.value)} required><option value="">Pilih produk</option>{items.map(x=><option key={x.id} value={x.id}>{x.name} — stok gudang {fmt(x.gudang_qty)} {x.unit}</option>)}</select></label>{selected&&<div className="hint">Stok saat ini: <b>{fmt(selected.gudang_qty)} {selected.unit}</b></div>}<label>Stok gudang setelah revisi<input type="number" min="0" step="0.001" value={qty} onChange={e=>setQty(e.target.value)} required/></label><label>Alasan / Catatan Revisi<input value={note} onChange={e=>setNote(e.target.value)} placeholder="Contoh: Koreksi hasil pengecekan fisik" required/></label><button disabled={saving||loading}>{saving?'Menyimpan…':'✓ Simpan Revisi Stok'}</button>{msg&&<div className={msg.includes('berhasil')?'success':'error'}>{msg}</div>}</form></Page>}
...
function Transfer({storeId}){
 ...
 const invalid=!!selected&&(amount<=0||amount>selected.gudang_qty);
 async function submit(e){
  ...
  if(amount>selected.gudang_qty)return setMsg('Jumlah melebihi stok gudang yang tersedia.');
  setSaving(true);
  const {error}=await supabase.rpc('record_stock_transfer',{p_store_id:storeId,p_product_id:productId,p_qty:amount,p_note:note.trim()||null});
 ...
 return <Page title="Transfer Stok" subtitle="Gudang → Operasional">
  <div className="notice">Transfer tercatat otomatis ke riwayat dan mengurangi stok gudang sekaligus menambah stok operasional.</div>
  <form className="card form" onSubmit={submit}>
   ...
   {selected&&<div className="stock-preview"><div><small>STOK GUDANG</small><b>{fmt(selected.gudang_qty)} {selected.unit}</b></div><div>→</div><div><small>STOK OPERASIONAL</small><b>{fmt(selected.operasional_qty)} {selected.unit}</b></div></div>}
   ...
   <button disabled={saving||loading||invalid}>{saving?'Menyimpan…':'Transfer Stok'}</button>
...
function CrewStock({storeId}){
 ...
 return <Page title="Stok" subtitle="Cek stok dengan cepat">
  ...
  <div className="crew-stock-list">{loading?<div className="empty">Memuat stok…</div>:items.length===0?<div className="empty">Belum ada produk aktif.</div>:items.map(x=>{
   ...
   return <div className={'crew-stock-card '+(isLow?'is-low':'')} key={x.id}><div className="crew-product"><b>{x.name}</b><small>{x.code} · {x.unit}</small></div><div className="crew-stock-values"><div><span>Gudang</span><strong>{fmt(x.gudang_qty)}</strong></div><div><span>Operasional</span><strong>{fmt(x.operasional_qty)}</strong></div></div><span className={'badge '+(isLow?'danger':'ok')}>{isLow?'Menipis':'Normal'}</span></div>})}</div>
 </Page>
}

function CrewOpname({storeId}){
 const {items,loading,reload}=useStock(storeId); const [productId,setProductId]=useState(''),[physical,setPhysical]=useState(''),[waste,setWaste]=useState('0'),[saving,setSaving]=useState(false),[msg,setMsg]=useState('');
 const selected=items.find(x=>x.id===productId);
 async function submit(e){e.preventDefault();setMsg('');if(!selected)return setMsg('Pilih produk terlebih dahulu.');const qty=Number(physical),w=Number(waste||0);if(!Number.isFinite(qty)||qty<0)return setMsg('Sisa fisik tidak boleh kurang dari 0.');if(!Number.isFinite(w)||w<0)return setMsg('Waste tidak boleh kurang dari 0.');setSaving(true);const {error}=await supabase.rpc('record_operational_opname',{p_store_id:storeId,p_product_id:productId,p_physical_stock:qty,p_waste_qty:w,p_note:null});if(error)setMsg(error.message);else{setMsg('Sisa stok berhasil disimpan.');setPhysical('');setWaste('0');await reload()}setSaving(false)}
 return <Page title="Input Sisa" subtitle="Masukkan sisa stok yang kamu hitung"><div className="crew-tip">💡 Hitung stok fisik terlebih dahulu, lalu masukkan angkanya di sini.</div><form className="card form crew-form" onSubmit={submit}><label>1. Pilih Produk<select value={productId} onChange={e=>setProductId(e.target.value)} required><option value="">Pilih produk</option>{items.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>{selected&&<div className="crew-current"><span>Stok sistem</span><b>{fmt(selected.operasional_qty)} {selected.unit}</b></div>}<label>2. Sisa Fisik<input className="crew-big-input" type="number" min="0" step="0.001" value={physical} onChange={e=>setPhysical(e.target.value)} required placeholder="Masukkan jumlah sisa"/></label><details className="crew-optional"><summary>Tambah waste (opsional)</summary><label>Waste<input type="number" min="0" step="0.001" value={waste} onChange={e=>setWaste(e.target.value)}/></label></details><button className="crew-save" disabled={saving||loading}>{saving?'Menyimpan…':'✓ Simpan Sisa Stok'}</button>{msg&&<div className={msg.includes('berhasil')?'success':'error'}>{msg}</div>}</form></Page>
}
